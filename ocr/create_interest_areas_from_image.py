...
def recognize_text(image_path, tesseract_config='--psm 6 -l spa'):
    ...
    trial_id = os.path.splitext(os.path.basename(image_path))[0]
    ...
    df_word_chars = pd.DataFrame(columns=['Character', 'X_Start', 'Y_Start', 'X_End', 'Y_End',
                                         'Block_Number', 'Paragraph_Number', 'Line_Number',
                                         'Word_Number', 'Char_Number_in_Word', 'Word',
                                         'X_Center', 'Y_Center', 'assigned_line', 'trial_id'])
        ...
        if isinstance(row_word['text'], str) and row_word['text'].strip() and row_word['level'] == 5:
            ...
            for index_char, row_char in relevant_chars.iterrows():
                ...
                char_data = {
                    ...
                    'X_End': char_right,
                    'Y_End': char_bottom,
                    'Block_Number': int(row_word['block_num']),
                    'Paragraph_Number': int(row_word['par_num']),
                    'Line_Number': int(row_word['line_num']),
                    'Word_Number': int(row_word['word_num']),
                    'Char_Number_in_Word': char_index_in_word,
                    'Word': word_text,
                    'X_Center': (char_left + char_right) / 2,
                    'Y_Center': (char_top + char_bottom) / 2,
                    'assigned_line': None,
                    'trial_id': trial_id
                }
            ...
            for index_space, row_space in spaces_following_word.iterrows():
                space_data = {
                    'Character': ' ',
                    'X_Start': int(row_space['left']),
                    'Y_Start': int(row_space['top']),
                    'X_End': int(row_space['left']) + int(row_space['width']),
                    'Y_End': int(row_space['top']) + int(row_space['height']),
                    'Block_Number': int(row_space['block_num']),
                    'Paragraph_Number': int(row_space['par_num']),
                    'Line_Number': int(row_space['line_num']),
                    'Word_Number': int(row_space['word_num']),
                    'Char_Number_in_Word': char_index_in_word,
                    'Word': word_text,
                    'X_Center': (int(row_space['left']) + int(row_space['left']) + int(row_space['width'])) / 2,
                    'Y_Center': (int(row_space['top']) + int(row_space['top']) + int(row_space['height'])) / 2,
                    'assigned_line': None,
                    'trial_id': trial_id
                }
    ...
    line_counter = 1
    for block_num in sorted(df_word_chars['Block_Number'].unique()):
        for par_num in sorted(df_word_chars.loc[df_word_chars['Block_Number'] == block_num, 'Paragraph_Number'].unique()):
            for line_num in sorted(df_word_chars.loc[(df_word_chars['Block_Number'] == block_num) & (df_word_chars['Paragraph_Number'] == par_num), 'Line_Number'].unique()):
                line_mask = (df_word_chars['Line_Number'] == line_num) & (df_word_chars['Paragraph_Number'] == par_num) & (df_word_chars['Block_Number'] == block_num)
                df_word_chars.loc[line_mask, 'assigned_line'] = line_counter
                line_counter += 1

    # Adjust Y_Start and Y_End for all characters on the same line
    for assigned_line in df_word_chars['assigned_line'].unique():
        line_mask = (df_word_chars['assigned_line'] == assigned_line)
        min_top = df_word_chars.loc[line_mask, 'Y_Start'].min()
        max_bottom = df_word_chars.loc[line_mask, 'Y_End'].max()
        df_word_chars.loc[line_mask, 'Y_Start'] = min_top
        df_word_chars.loc[line_mask, 'Y_End'] = max_bottom
...
def draw_char_boxes(image_path, df_word_chars, output_path='output_boxes_combined.png'):
    ...
    for index, row in df_word_chars.iterrows():
        left = int(row['X_Start'])
        top = int(row['Y_Start'])
        right = int(row['X_End'])
        bottom = int(row['Y_End'])
